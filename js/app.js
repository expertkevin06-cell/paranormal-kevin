import { S, save, view, runtime } from './state.js';
...
import { saveCanvas, stamp, startVideo, listMedia, delMedia, share, download } from './capture.js';
import { makeReport, flushQueue } from './ai.js';
import { initNet, onNet, effQuality } from './net.js';
import { initSensors, initMotion, Sens } from './sensors.js';
import { EVP } from './evp.js';
import { Anom, pushSample, dev, updateScore, logEvent, cooled } from './anomaly.js';
...
async function buildCamControls(){ const tr=rgbv.srcObject?.getVideoTracks?.()[0]; if(!tr) return;
  ...
  const mk=(label,key,min,max,step)=>{ const l=document.createElement('label');
    l.innerHTML=`${label} <output></output><input type="range" min="${min}" max="${max}" step="${step}">`;
    const inp=l.querySelector('input'); inp.value=S.cam[key]||min; l.querySelector('output').textContent=inp.value;
    inp.oninput=()=>{ S.cam[key]=+inp.value; l.querySelector('output').textContent=inp.value; save();
      tr.applyConstraints?.({advanced:[{[key]:+inp.value}]}).catch(()=>{}); };
    box.appendChild(l); };
  ...
  if(cap.torch){ const l=document.createElement('label'); l.className='chk';
...
function loop(){ requestAnimationFrame(loop); tick++; if(!cv.width) fit();
  ...
  if(S.ai.detect && rgbReady() && tick%30===0) detect(rgbv).then(b=>{ runtime.boxes=b; presenceCheck(b); }).catch(()=>{});
  if(tick%10===0 && rgbReady()){ const g=grayOf(rgbv,64,48);
    if(prevGray){ let s=0; for(let i=0;i<g.length;i++) s+=Math.abs(g[i]-prevGray[i]);
      runtime.motion=s/g.length; }
    prevGray=g; }
...
const sev=(z,thr)=> z>thr*2?3 : z>thr*1.4?2 : 1;
...
function sampleAnomalies(){ if(!S.anomaly.enabled){ updateScore({}); return; }
  ...
  let zMag=0; if(Sens.mag){ pushSample('mag',Sens.mag); zMag=Math.abs(dev('mag',Sens.mag));
    if(zMag>S.anomaly.magSd && cooled('mag')) fire('ANOMALIE MAGNÉTIQUE', sev(zMag,S.anomaly.magSd), `${Sens.mag.toFixed(0)} µT (z=${zMag.toFixed(1)}) — câblage proche ?`); }
  ...
  let pN=0; if(S.layers.depth){ pushSample('prox',runtime.near);
    ...
    if(b!=null && runtime.near>S.anomaly.prox && b<S.anomaly.prox/2 && cooled('prox')){
      ...
      fire('APPROCHE (LiDAR)',2,`Fraction proche ${(runtime.near*100)|0}% — personne/objet entré dans le champ`); } }
...
async function fire(type,severity,detail,extra={}){
  const snap=makeSnap();
  const ev=await logEvent(type,severity,detail,{snapshot:snap,...extra});
...
bind('palette',()=>S.palette,v=>S.palette=v); bind('unit',()=>S.unit,v=>S.unit=v);
bind('emissivity',()=>S.emissivity,v=>S.emissivity=v); bind('agc',()=>S.agc,v=>S.agc=v);
bind('level',()=>S.level,v=>S.level=v); bind('span',()=>S.span,v=>S.span=v);
bind('Tmin',()=>S.Tmin,v=>S.Tmin=v); bind('Tmax',()=>S.Tmax,v=>S.Tmax=v);
bind('anomalyOn',()=>S.anomaly.enabled,v=>S.anomaly.enabled=v);
bind('night',()=>S.night,v=>S.night=v); bind('aiDetect',()=>S.ai.detect,v=>S.ai.detect=v);
...
['stack','denoise','sharpen','clahe','sat','vibrance','contrast','gamma','kelvin','vignette','detailTransfer']
...
bind('llmProvider',()=>S.llm.provider,v=>S.llm.provider=v); bind('llmKey',()=>S.llm.key,v=>S.llm.key=v);
...
$('#sUvc').onclick=async()=>{ $('#srcDlg').close();
  ...
  try{ await th.connectUVC(ext?.deviceId);
...
$('#bPhoto').onclick=async()=>{ if(runtime.processing) return; flash();
  ...
  if(S.layers.rgb && rgbReady() && q.stack>1){
    ...
    try{ const img=await captureStack(rgbv,q.stack); const P=processPhoto(img,S.expert);
      const out=document.createElement('canvas'); out.width=P.width*S.photo.scale/2; out.height=P.height*S.photo.scale/2;
      const g=out.getContext('2d');
      renderScene(g,out.width,out.height,{rgb:P, th:S.layers.th?th.frame:null, depth:S.layers.depth?runtime.depth:null},{live:false});
      stamp(g,out.width,out.height); await saveCanvas(out,{stack:q.stack});
      toast('Photo expert enregistrée'); }finally{ runtime.processing=false; }
...
$('#bEvp').onclick=async()=>{ if(evp.on){ evp.stop(); spec.hidden=true; $('#bEvp').classList.remove('on');
    $('#sEvp').textContent='🎙 off'; toast('EVP stoppé'); return; }
...
$('#bGallery').onclick=async()=>{ $('#gallery').classList.add('open');
  ...
  for(const it of items.sort((a,b)=>b.ts-a.ts)){ const url=URL.createObjectURL(it.blob);
    ...
    w.onclick=e=>{ if(e.detail===2){ delMedia('media',it.id).then(()=>$('#bGallery').onclick()); }
