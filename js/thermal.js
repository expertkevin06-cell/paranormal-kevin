...
export class Thermal {
  ...
  startDemo(){ this.stop().then(()=>{ const w=S.usb.w||256,h=S.usb.h||192;
    ...
    this._timer=setInterval(()=>{ if(this.paused) return; t+=0.033;
      ...
      const spots=[[0.3+0.15*Math.sin(t*0.7),0.4+0.1*Math.cos(t*0.5),90,0.10],
                   [0.7,0.65,55,0.14],[0.5+0.2*Math.cos(t*0.3),0.25,38,0.08],
                   [0.15+0.05*Math.sin(t*2.7),0.8,12,0.05]];
